...
class MultiChannelSale(models.Model):
	...
	def update_woocommerce_orders(self, woocommerce=False):
		...
		error_orders= self.env['order.feed'].search([('state','=','error')]).mapped("store_id")
		records = self.env['channel.order.mappings'].search([('channel_id.id','=',self.id)]).mapped("store_order_id")
		if error_orders:
