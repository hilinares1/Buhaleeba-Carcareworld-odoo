...
class MultiChannelSale(models.Model):
	...
	def import_woocommerce_types(self):
		message =''
		list_category = []
		woocommerce = self.get_woocommerce_connection()
		if self.id:
			category_map_data = self.env['channel.type.mappings']
			count = 0
			i=1
			while(i):
				cat_url = 'products/ysg_product_type?page='+str(i)
				category_data = woocommerce.get(cat_url).json()
				if category_data:
					count += self.import_woocommerce_all_types(cat_url,False)
					i +=1
				else:
					i=0
			message += str(count)+" Types Imported!"
	...
	def import_woocommerce_all_types(self,cat_url,parent_id=False):
		message =''
		list_category = []
		category_map_data = self.env['channel.type.mappings']
		count = 0
		...
		if not cat_url and parent_id:
			cat_url = 'products/ysg_product_type'
			# if parent_id and not isinstance(parent_id,(dict)):
			cat_url = cat_url+"/"+str(parent_id)
		category_data = woocommerce.get(cat_url).json()
		if isinstance(category_data,dict):
			...
			category_data = [category_data,]
		for category in category_data:
			if category['parent'] and not category_map_data.search([('store_type_id','=',category['parent']),('channel_id.id','=',self.id)]):
				self.import_woocommerce_all_categories(False,category['parent'])
			if not category_map_data.search([('store_type_id','=',category['id']),('channel_id.id','=',self.id)]) and not self.env['type.feed'].search([('store_id','=',category['id']),('channel_id.id','=',self.id)]):
				category_search_record = self.env['product.type'].search([('name','=',category['name']),('channel_type_ids.instance_id.id','=',self.id)])
				if category_search_record:
					mapping_dict = {
								'channel_id'		: self.id,
								'store_type_id'	: category['id'],
								'odoo_type_id'	: category_search_record.id,
								'category_name'		: category_search_record.id,
					}
					obj = self.env['channel.category.mappings']
					self._create_mapping(obj, mapping_dict)
					...
					category_rec = self.env['type.feed'].create(category_dict)
					self._cr.commit()
					list_category.append(category_rec)
		feed_res = dict(create_ids=list_category,update_ids=[])
		self.env['channel.operation'].post_feed_import_process(self,feed_res)
