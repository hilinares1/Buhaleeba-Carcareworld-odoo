...
class AccountEntryAnalysis(models.Model):
    _name = "account.entry.analysis"
    _description = "Account Entry Analysis"
    _auto = False
    _rec_name = 'date'
    _order = 'date desc'
    ...
    move_id = fields.Many2one('account.move', oldname="invoice")
    ...
    def init(self):
        cr = self.env.cr
        tools.drop_view_if_exists(cr, 'account_entry_analysis')
        cr.execute("""
            create or replace view account_entry_analysis as (
                SELECT
                	mvl.id,
