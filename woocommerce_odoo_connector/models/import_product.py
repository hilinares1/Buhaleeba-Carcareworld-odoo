		...
		for variant_id in variation_ids:
			...
			if variant['attributes']:
				...
				for attributes in variant['attributes']:
					...
					attrib_value_id = self.env['channel.attribute.value.mappings'].search([('store_attribute_value_name','=',attributes['option']),('channel_id.id','=',self.id),('attribute_value_name.attribute_id.id','=',attrib_name_id.attribute_name.id)])
					attr = {}
					...
					if product_data:
						...
						for product in product_data:
							...
							if not self.env['channel.template.mappings'].search([('store_product_id','=',product['id']),('channel_id.id','=',self.id)]):
								...
								count = count + 1
								for category in product['categories']:
									category_id = self.env['category.feed'].search([('store_id','=',category["id"]),('channel_id.id','=',self.id)])
									if category_id:
