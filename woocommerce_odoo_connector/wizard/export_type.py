...
class ExportWoocommerceType(models.TransientModel):
    _inherit = "export.type"

    def action_woocommerce_export_type(self):
    ...
    def woocommerce_export_now(self, record, initial_record_id):
        ...
        if with_product:
            cat_mapped = self.env["channel.type.mappings"].search([
                ("channel_id","=",channel.id),
                ("odoo_type_id","=",record.id)
                ])
            if cat_mapped:
                return [True,{"id":cat_mapped.store_type_id}]
        cat_id = self.woocommerce_sync_type(
            channel, woocommerce, record, initial_record_id, with_product)
        if cat_id:
            return_list = [True, {"id":cat_id}]
        return return_list

    def woocommerce_sync_type(self, channel, woocommerce, record, initial_record_id, with_product):
        p_cat_id = 0
        res = ''
        parent_id = record.parent_id
        if parent_id.id:
            is_parent_mapped = self.env['channel.type.mappings'].search([
                ("channel_id", "=", channel.id),
                ("odoo_type_id", '=', parent_id.id)
            ])
            if not is_parent_mapped:
                ...
                p_cat_id = is_parent_mapped.store_type_id
        res = self.woocommerce_create_type(
            woocommerce, channel, record, initial_record_id, p_cat_id, with_product)
        return res

    def woocommerce_create_type(self, woocommerce, channel, record, initial_record_id, p_cat_id, with_product=False):
        returnid = False
